...
export class AuthService {
  ...
  async register(dto: RegisterDto, ipAddress: string, userAgent: string) {
    ...
    // Dispatch OTP email in background for instant HTTP response (<50ms)
    setImmediate(() => {
  ...
  async forgotPassword(email: string) {
    ...
    // Dispatch reset email in background for instant HTTP response (<50ms)
    setImmediate(() => {
  ...
  async resendVerification(email: string) {
    ...
    // Dispatch verification OTP email in background for instant HTTP response (<50ms)
    setImmediate(() => {
  ...
  async getUserProfile(userId: number) {
    ...
    const ROLE_TIER_MAPPING = {
      SUPER_ADMIN: 'DIAMOND', CEO: 'DIAMOND', DEVELOPER: 'DIAMOND', ASSISTANT_CEO: 'DIAMOND',
      ADMIN: 'DIAMOND', EDITOR_ADMIN: 'DIAMOND', SALE_ADMIN: 'DIAMOND',
      EXPERT: 'DIAMOND', EDITOR_PRO: 'DIAMOND', EDITOR: 'DIAMOND', SALE: 'DIAMOND',
      CLIENT_VIP: 'GOLD',
    };
