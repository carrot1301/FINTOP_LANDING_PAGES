...
let PermissionsGuard = class PermissionsGuard {
    ...
    canActivate(context) {
        ...
        const isExecutive = user.roles?.some((r) => r === 'CEO' || r === 'DEVELOPER' || r === 'SUPER_ADMIN' || r === 'ASSISTANT_CEO');
        if (isExecutive) {
            return true;
        }
        const hasPermission = requiredPermissions.every((perm) => user.permissions.includes(perm));
        if (!hasPermission) {
            throw new common_1.ForbiddenException('Insufficient specific permissions');
        }
        return true;
    }
};
