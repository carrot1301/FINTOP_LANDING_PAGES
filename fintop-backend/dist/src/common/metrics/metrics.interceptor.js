    ...
    intercept(context, next) {
        ...
        return next.handle().pipe((0, operators_1.tap)({
            next: () => {
                ...
                this.metricsService.httpRequestDuration.observe({ method: request.method, route, status_code: statusCode }, duration);
                this.metricsService.httpRequestsTotal.inc({ method: request.method, route, status_code: statusCode });
            },
            error: () => {
                const duration = (Date.now() - startTime) / 1000;
                this.metricsService.httpRequestDuration.observe({ method: request.method, route, status_code: '500' }, duration);
