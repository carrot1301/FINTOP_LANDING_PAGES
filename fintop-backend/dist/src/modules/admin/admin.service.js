...
let AdminService = class AdminService {
    ...
    async onModuleInit() {
        ...
        try {
            await this.prisma.user.updateMany({
                ...
                data: {
                    deletedAt: new Date(),
                    status: client_1.RECORD_STATUS.INACTIVE,
                },
            });
            await this.prisma.invoice.updateMany({
                where: { status: 'PAID' },
                data: { status: client_1.INVOICE_STATUS.DRAFT },
            });
    ...
    static CEO_EMAILS = ['fintop.bashare@gmail.com', 'fintop.ba@gmail.com'];
    static CEO_EMAIL = 'fintop.bashare@gmail.com';
    async enforceRoleHierarchy(targetUserId, adminId, action) {
        ...
        const isTargetCeo = AdminService_1.CEO_EMAILS.includes(targetUser.email) || targetRoleCodes.includes('CEO') || targetRoleCodes.includes('SUPER_ADMIN');
        const isAdminCeo = AdminService_1.CEO_EMAILS.includes(adminUser.email);
        if (isTargetCeo && !isAdminCeo) {
            throw new common_1.BadRequestException(`Không thể ${action} tài khoản CEO (${targetUser.email}). Chỉ chính tài khoản CEO Tối cao mới có quyền thực hiện.`);
        }
        ...
        const isAllowedToModifyRoles = adminRoleCodes.some(c => ['CEO', 'DEVELOPER', 'SUPER_ADMIN', 'ASSISTANT_CEO'].includes(c)) || AdminService_1.CEO_EMAILS.includes(adminUser.email);
        if (action.includes('vai trò') && !isAllowedToModifyRoles) {
            throw new common_1.BadRequestException(`Chỉ CEO, Developer và Trợ lý CEO mới có quyền chỉnh sửa/thay đổi vai trò phân quyền của người dùng.`);
        }
    }
    async syncClientRoleForTier(userId, tierLevel, adminId = 1) {
        try {
            ...
            if (!hasStaffRole) {
                ...
                if (tierLevel === client_1.SUBSCRIPTION_TIER.SILVER)
                    targetRoleCode = 'CLIENT_PRO';
                else if (tierLevel === client_1.SUBSCRIPTION_TIER.GOLD)
                    targetRoleCode = 'CLIENT_VIP';
                else if (tierLevel === client_1.SUBSCRIPTION_TIER.DIAMOND)
                    targetRoleCode = 'CLIENT_DIAMOND';
                ...
                if (!currentRoleCodes.includes(targetRoleCode)) {
                    ...
                    if (targetRole) {
                        await this.prisma.userRole.deleteMany({
                            where: {
                                userId,
                                role: { code: { in: clientRoleCodes.filter(c => c !== targetRoleCode) } },
                            },
                        });
    ...
    async getUsers(page = 1, limit = 20, search, status, userType, tierLevel, roleCode) {
    ...
    async assignRole(userId, roleCode, adminId, staffCode) {
        ...
        if (clientRoleCodes.includes(roleCode)) {
            await this.prisma.userRole.deleteMany({
                where: {
                    userId,
                    role: { code: { in: clientRoleCodes } },
                },
            });
            let tierLevel = client_1.SUBSCRIPTION_TIER.STANDARD;
            if (roleCode === 'CLIENT_PRO')
                tierLevel = client_1.SUBSCRIPTION_TIER.SILVER;
            else if (roleCode === 'CLIENT_VIP')
                tierLevel = client_1.SUBSCRIPTION_TIER.GOLD;
            else if (roleCode === 'CLIENT_DIAMOND')
                tierLevel = client_1.SUBSCRIPTION_TIER.DIAMOND;
            await this.prisma.user.update({
                where: { id: userId },
                data: { tierLevel },
            });
        ...
        if (!role) {
            const foundPredefined = AdminService_1.PREDEFINED_12_ROLES.find(r => r.code === roleCode);
        ...
        if (dto.tierLevel !== undefined) {
            ...
            if (['STANDARD', 'SILVER', 'GOLD', 'DIAMOND'].includes(tierUpper)) {
                ...
                if (!hasStaffRole) {
                    ...
                    if (!targetRole) {
                        const foundPredefined = AdminService_1.PREDEFINED_12_ROLES.find(r => r.code === targetRoleCode);
    ...
    async getRoles() {
        ...
        try {
            const expertRole = await this.prisma.role.findFirst({ where: { code: 'EXPERT' } });
            if (expertRole) {
        ...
        for (const r of AdminService_1.PREDEFINED_12_ROLES) {
        ...
        try {
            ...
            if (userPerms.length > 0) {
                ...
                const dbRoles = await this.prisma.role.findMany({
                    where: { code: { in: fullAccessRoles }, deletedAt: null },
                });
    ...
    async getReports(page = 1, limit = 20) {
        ...
        const mapped = reports.map(r => ({
            ...
            language: r.language || 'Tiếng Việt',
            status: r.status,
    ...
    async createReport(dto, adminId) {
    ...
    async updateReport(id, dto, adminId) {
        ...
        if (dto.fileSize !== undefined)
            data.fileSize = parseInt(dto.fileSize, 10);
        if (dto.minTierAccess !== undefined)
            data.minTierAccess = dto.minTierAccess;
        if (dto.status !== undefined)
            data.status = dto.status;
